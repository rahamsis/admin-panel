...
import { WebSite, Banner } from "@/types/webSite";
...
export async function getBanners(tenant: string) {
  try {
    const response = await fetch(`${process.env.APP_BACK_END}/get-banners`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        "X-Tenant-ID": tenant,
    ...
    throw new Error("Error al traer los banner")
  }
}

export async function insertBanners(tenant: string, formData: FormData) {
  try {
    const response = await fetch(`${process.env.APP_BACK_END}/create-banners`, {
      method: 'POST',
      headers: {
        // 'Content-Type': 'application/json',
        "X-Tenant-ID": tenant,
...
export async function removeBanners(tenant: string, idBanner: String) {
  try {
    const response = await fetch(`${process.env.APP_BACK_END}/delete-banners?idBanner=${idBanner}`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
        "X-Tenant-ID": tenant,
